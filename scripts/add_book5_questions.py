#!/usr/bin/env python3
"""Append the reviewed Book 5 question set from the supplied assessment DOCX."""
import json
import re
...
ANSWERS = [
    list('BCCDD'), list('ADABD'), list('CDCDD'), list('BDBBB'), list('DDCBA'),
    list('CBABD'), list('DBCAA'), list('ABACC'), list('DAAAD'), list('BCCCB'),
]
META = {
...
def build_book():
    items = paragraphs()
    start = next(index for index, text in enumerate(items) if text == 'Book 5  |  Night of the Ninjas')
    stop = next(index for index in range(start + 1, len(items)) if items[index].startswith('Book 6  |'))
    ...
    for chapter_number in range(1, 11):
        ...
        for question_number in range(1, 6):
            ...
            questions.append({
                ...
                'id': f'B5C{chapter_number}Q{question_number}', 'difficulty': difficulty,
            })
        chapters.append({'book': 5, 'chapter': chapter_number, 'chapter_title': title, 'questions': questions})
    return {'book': 5, 'book_title': 'Night of the Ninjas', 'book_title_cn': '忍者的秘密', 'chapters': chapters}


def main():
    data = json.loads(TARGET.read_text(encoding='utf-8'))
    book = build_book()
    data['books'] = [item for item in data['books'] if item.get('book') != 5] + [book]
    TARGET.write_text(json.dumps(data, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
    print('questions.json: Book 5 appended, 10 chapters / 50 questions')
