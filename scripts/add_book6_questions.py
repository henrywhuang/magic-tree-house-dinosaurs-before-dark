#!/usr/bin/env python3
"""Append the reviewed Book 6 question set from the supplied assessment DOCX."""
import json
import re
...
ANSWERS = [
    list('BDCBC'), list('BABDC'), list('CADCB'), list('CBADA'), list('CBAAA'),
    list('AABDA'), list('AAACA'), list('DDBBC'), list('DDDAA'), list('ABDBB'),
]
META = {
...
def build_book():
    items = paragraphs()
    start = next(index for index, text in enumerate(items) if text == 'Book 6  |  Afternoon on the Amazon')
    stop = next(index for index in range(start + 1, len(items)) if items[index].startswith('Book 7  |'))
    ...
    for chapter_number in range(1, 11):
        ...
        for question_number in range(1, 6):
            ...
            questions.append({
                ...
                'id': f'B6C{chapter_number}Q{question_number}', 'difficulty': difficulty,
            })
        chapters.append({'book': 6, 'chapter': chapter_number, 'chapter_title': title, 'questions': questions})
    return {'book': 6, 'book_title': 'Afternoon on the Amazon', 'book_title_cn': '亚马孙探险', 'chapters': chapters}


def main():
    data = json.loads(TARGET.read_text(encoding='utf-8'))
    book = build_book()
    data['books'] = [item for item in data['books'] if item.get('book') != 6] + [book]
    TARGET.write_text(json.dumps(data, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
    print('questions.json: Book 6 appended, 10 chapters / 50 questions')
