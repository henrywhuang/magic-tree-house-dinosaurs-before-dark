#!/usr/bin/env python3
"""Append the reviewed Book 7 question set from the supplied assessment DOCX."""
import json
import re
...
ANSWERS = [
    list('ABBAC'), list('DDCCB'), list('DDABA'), list('DDDBC'), list('CBBAB'),
    list('CCBAA'), list('DABBD'), list('BCAAA'), list('ACBBA'), list('BDBAA'),
]
META = {
...
def build_book():
    items = paragraphs()
    start = next(index for index, text in enumerate(items) if text == 'Book 7  |  Sunset of the Sabertooth')
    stop = next(index for index in range(start + 1, len(items)) if items[index].startswith('Book 8  |'))
    ...
    for chapter_number in range(1, 11):
        ...
        for question_number in range(1, 6):
            ...
            questions.append({
                ...
                'id': f'B7C{chapter_number}Q{question_number}', 'difficulty': difficulty,
            })
        chapters.append({'book': 7, 'chapter': chapter_number, 'chapter_title': title, 'questions': questions})
    return {'book': 7, 'book_title': 'Sunset of the Sabertooth', 'book_title_cn': '勇斗剑齿虎', 'chapters': chapters}


def main():
    data = json.loads(TARGET.read_text(encoding='utf-8'))
    book = build_book()
    data['books'] = [item for item in data['books'] if item.get('book') != 7] + [book]
    TARGET.write_text(json.dumps(data, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
    print('questions.json: Book 7 appended, 10 chapters / 50 questions')
