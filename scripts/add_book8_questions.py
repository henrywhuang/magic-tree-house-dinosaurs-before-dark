#!/usr/bin/env python3
"""Append the reviewed Book 8 question set from the supplied assessment DOCX."""
import json
import re
...
ANSWERS = [
    list('BDBBC'), list('BBDDD'), list('CDCBA'), list('DCDCD'), list('AACDB'),
    list('DCCAA'), list('CBBAD'), list('CABAD'), list('BDABC'), list('DBABC'),
]
META = {
...
def build_book():
    items = paragraphs()
    start = next(index for index, text in enumerate(items) if text == 'Book 8  |  Midnight on the Moon')
    stop = next(index for index in range(start + 1, len(items)) if items[index].startswith('Book 9  |'))
    ...
    for chapter_number in range(1, 11):
        ...
        for question_number in range(1, 6):
            ...
            questions.append({
                ...
                'id': f'B8C{chapter_number}Q{question_number}', 'difficulty': difficulty,
            })
        chapters.append({'book': 8, 'chapter': chapter_number, 'chapter_title': title, 'questions': questions})
    return {'book': 8, 'book_title': 'Midnight on the Moon', 'book_title_cn': '月球漫游记', 'chapters': chapters}


def main():
    data = json.loads(TARGET.read_text(encoding='utf-8'))
    book = build_book()
    data['books'] = [item for item in data['books'] if item.get('book') != 8] + [book]
    TARGET.write_text(json.dumps(data, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
    print('questions.json: Book 8 appended, 10 chapters / 50 questions')
