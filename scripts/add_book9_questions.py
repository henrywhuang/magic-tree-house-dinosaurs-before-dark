#!/usr/bin/env python3
"""Append the reviewed Book 9 question set from the supplied assessment DOCX."""
import json
import re
...
ANSWERS = [
    list('CCBBC'), list('BADBD'), list('CDBDA'), list('ACDCC'), list('CBBCD'),
    list('CDBAD'), list('AAAAA'), list('AACBD'), list('ACADC'), list('CCCDD'),
]
META = {
...
def build_book():
    items = paragraphs()
    start = next(index for index, text in enumerate(items) if text == 'Book 9  |  Dolphins at Daybreak')
    stop = next(index for index in range(start + 1, len(items)) if items[index].startswith('Book 10  |'))
    ...
    for chapter_number in range(1, 11):
        ...
        for question_number in range(1, 6):
            ...
            questions.append({
                ...
                'id': f'B9C{chapter_number}Q{question_number}', 'difficulty': difficulty,
            })
        chapters.append({'book': 9, 'chapter': chapter_number, 'chapter_title': title, 'questions': questions})
    return {'book': 9, 'book_title': 'Dolphins at Daybreak', 'book_title_cn': '与海豚共舞', 'chapters': chapters}


def main():
    data = json.loads(TARGET.read_text(encoding='utf-8'))
    book = build_book()
    data['books'] = [item for item in data['books'] if item.get('book') != 9] + [book]
    TARGET.write_text(json.dumps(data, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
    print('questions.json: Book 9 appended, 10 chapters / 50 questions')
