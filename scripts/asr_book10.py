#!/usr/bin/env python3
"""Transcribe Book 10 chapter audio to word-level timestamps."""
import argparse
import json
...
for chapter in chapters:
    source = ROOT / 'assets' / 'audio' / f'book-10-chapter-{chapter:02d}.mp3'
    if not source.exists():
        print(f'skip missing {source.name}', flush=True)
        continue
    target = OUT / f'book10-ch{chapter:02d}.json'
    segments, info = model.transcribe(
        ...
        initial_prompt='A chapter from Magic Tree House: Ghost Town at Sundown, American English narration.',
    )
    words = [
