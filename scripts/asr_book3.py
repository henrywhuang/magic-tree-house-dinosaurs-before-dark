...
for c in range(1,11):
    mp3=f"assets/audio/book-03-chapter-{c:02d}.mp3"; out=f"tmp/asr/book3-ch{c:02d}.json"
    segments, info = model.transcribe(mp3, language="en", word_timestamps=True,
        vad_filter=True, vad_parameters=dict(min_silence_duration_ms=300), beam_size=5, temperature=0.0,
...
print("BOOK3 ASR DONE", flush=True)
