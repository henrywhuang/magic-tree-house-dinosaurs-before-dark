#!/usr/bin/env python3
"""Transcribe Book 6 chapter audio to word-level timestamps."""
import json
...
for chapter in range(1, 11):
    source = ROOT / 'assets' / 'audio' / f'book-06-chapter-{chapter:02d}.mp3'
    if not source.exists():
        print(f'skip missing {source.name}', flush=True)
        continue
    target = OUT / f'book6-ch{chapter:02d}.json'
    segments, info = model.transcribe(
        ...
        initial_prompt="A chapter from Magic Tree House: Afternoon on the Amazon, American English narration.",
    )
    words = [
