#!/usr/bin/env python3
"""Transcribe Book 7 chapter audio to word-level timestamps."""
import json
...
for chapter in range(1, 11):
    source = ROOT / 'assets' / 'audio' / f'book-07-chapter-{chapter:02d}.mp3'
    if not source.exists():
        print(f'skip missing {source.name}', flush=True)
        continue
    target = OUT / f'book7-ch{chapter:02d}.json'
    segments, info = model.transcribe(
        ...
        initial_prompt="A chapter from Magic Tree House: Sunset of the Sabertooth, American English narration.",
    )
    words = [
