#!/usr/bin/env python3
"""Transcribe Book 8 chapter audio to word-level timestamps."""
import json
...
for chapter in range(1, 11):
    source = ROOT / 'assets' / 'audio' / f'book-08-chapter-{chapter:02d}.mp3'
    if not source.exists():
        print(f'skip missing {source.name}', flush=True)
        continue
    target = OUT / f'book8-ch{chapter:02d}.json'
    segments, info = model.transcribe(
        ...
        initial_prompt='A chapter from Magic Tree House: Midnight on the Moon, American English narration.',
    )
    words = [
