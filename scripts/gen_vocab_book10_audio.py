#!/usr/bin/env python3
"""Generate Book 10 vocabulary sentence clips with local TTS."""
import glob
import json
import os
import subprocess
...
def collect():
    result = []
    for filename in sorted(glob.glob('data/vocabulary-book-10-chapter-*.json')):
...
def main():
    items = collect()
    print(f'Generating {len(items)} Book 10 sentence clips...', flush=True)
