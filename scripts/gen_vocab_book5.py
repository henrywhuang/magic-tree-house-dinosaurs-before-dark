#!/usr/bin/env python3
"""Generate Book 5 vocabulary learning data with the Book 4 schema."""
import json
import re
import sys
...
from book5_content import TITLES


def build_chapter(chapter):
    data = json.loads((ROOT / f'data/book-05-chapter-{chapter:02d}.json').read_text(encoding='utf-8'))
    words = data['words']
    ...
    for surface in order:
        ...
        entry = {
            ...
            'sentenceAudio': f'assets/audio/sentences/book-05-chapter-{chapter:02d}/{word_id}.mp3',
        }
        if surface in core:
    ...
    return {'book': 5, 'chapter': chapter, 'title': TITLES[chapter - 1], 'words': entries}


def main():
    for chapter in range(1, 11):
        result = build_chapter(chapter)
        target = ROOT / 'data' / f'vocabulary-book-05-chapter-{chapter:02d}.json'
