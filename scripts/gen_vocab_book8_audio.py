#!/usr/bin/env python3
"""Generate Book 8 vocabulary sentence clips with the local system TTS."""
import glob
import json
import os
import subprocess
...
def collect():
    result = []
    for filename in sorted(glob.glob('data/vocabulary-book-08-chapter-*.json')):
...
def main():
    items = collect()
    print(f'Generating {len(items)} Book 8 sentence clips...', flush=True)
