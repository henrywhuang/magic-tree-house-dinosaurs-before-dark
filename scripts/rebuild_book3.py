...
Timing: tmp/asr/book3-chXX.json (faster-whisper large-v3)
Output: tmp/rebuilt/book-03-chapter-XX.json (staging)
"""
...
BOOK = 2
TITLES = ['Meow!', 'Oh, Man. Mummies!', "It's Alive!", 'Back from the Dead', 'The Ghost-Queen',
          'The Writing on the Wall', 'The Scroll', 'The Mummy', 'Follow the Leader', 'Another Clue']
COVER = 'assets/images/cover.jpg'
SCENES = json.load(open('tmp/book3_qa/scenes.json'))  # {chapter_str: [3 scenes: title/desc/prompt]}
...
def build_chapter(ci, pages):
    ...
    asr = json.load(open(f'tmp/asr/book3-ch{ci+1:02d}.json'))
    ...
    sc = SCENES[str(ci + 1)]
    scenes = [[s['title'], f'assets/images/book-03-scenes/chapter-{ci+1:02d}-scene-{k+1:02d}.webp', s['desc']]
...
if __name__ == '__main__':
    ...
    for ci in chapters:
        ...
        json.dump(d, open(f'tmp/rebuilt/book-03-chapter-{ci+1:02d}.json', 'w'), ensure_ascii=False, indent=1)
