#!/usr/bin/env python3
"""Build Book 5 reader JSON from proofread OCR and ASR timestamps."""
import difflib
import json
import re
import sys
...
import rekey
from book5_content import SCENE_ANCHORS, SCENES, TITLES, VOCABULARY

SOURCE = ROOT / '5.忍着的秘密.pdf_by_PaddleOCR-VL-1.6.json'
CHAPTER_STARTS = [5, 13, 19, 27, 37, 43, 51, 61, 69, 79]
STORY_END = 87
...
def build_chapter(pages, chapter_index):
    ...
    asr = json.loads((ROOT / 'tmp' / 'asr' / f'book5-ch{chapter_index + 1:02d}.json').read_text())
    ...
    scenes = [
        [title, f'assets/images/book-05-scenes/chapter-{chapter_index + 1:02d}-scene-{scene_index + 1:02d}.webp', description]
...
def main():
    ...
    for chapter_index in range(10):
        ...
        target = ROOT / 'data' / f'book-05-chapter-{chapter_index + 1:02d}.json'
