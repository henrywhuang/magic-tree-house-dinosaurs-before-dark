#!/usr/bin/env python3
"""Build Book 7 reader JSON from proofread OCR and ASR timestamps."""
import difflib
import json
import re
import sys
...
import rekey
from book7_content import SCENE_ANCHORS, SCENES, TITLES, VOCABULARY

SOURCE = ROOT / '7.勇斗剑齿虎.pdf_by_PaddleOCR-VL-1.6.json'
CHAPTER_STARTS = [6, 13, 24, 29, 40, 47, 54, 64, 70, 78]
STORY_END = 87
...
def chapter_text(pages, chapter_index):
    ...
    for line in (part.strip() for part in text.splitlines() if part.strip()):
        if merged and line[:1].islower() and re.search(r'[A-Za-z]$', merged[-1]):
            ...
            merged.append(line)
    text = '\n'.join(merged)
    # A few page boundaries land before a proper name or inside one speaker's
    # quotation, so the generic lowercase-continuation rule cannot detect them.
    for broken, joined in (
        ('He followed\nAnnie into the Frog Creek woods.',
         'He followed Annie into the Frog Creek woods.'),
        ('"We better find the third M thing soon," said\nJack.',
         '"We better find the third M thing soon," said Jack.'),
        ('“Maybe we should give them our towels,” said\nAnnie.',
         '“Maybe we should give them our towels,” said Annie.'),
        ('"Someone\'s sleeping back there.\nHear him snoring?"',
         '"Someone\'s sleeping back there. Hear him snoring?"'),
    ):
        text = text.replace(broken, joined)
    text = re.sub(r'(?m)^90000\n?', '', text)
    text = re.sub(r'\$\s*\^\{?\d+\}?\s*\$', '', text)
    text = text.replace('thinq', 'thing').replace('teenny', 'teeny')
    text = text.replace('Cro. Magnons', 'Cro-Magnons')
    # The OCR alternates straight and typographic quotation marks.  Normalize
    # them as paired opening/closing marks, and normalize contraction apostrophes.
    normalized, quote_open = [], False
...
def scene_boundaries(paragraphs, chapter_index):
    ...
    for anchor in SCENE_ANCHORS[chapter_index]:
        # Scene briefs predate punctuation normalization, so compare quote and
        # apostrophe variants as the same text while retaining every word.
        anchor_key = re.sub(r'[“”"\'’]', '', anchor)
        boundary = next((index for index, paragraph in enumerate(paragraphs)
                         if anchor_key in re.sub(r'[“”"\'’]', '', paragraph)), None)
        if boundary is None:
...
def build_chapter(pages, chapter_index):
    ...
    asr = json.loads((ROOT / 'tmp' / 'asr' / f'book7-ch{chapter_index + 1:02d}.json').read_text())
    ...
    scenes = [
        [title, f'assets/images/book-07-scenes/chapter-{chapter_index + 1:02d}-scene-{scene_index + 1:02d}.webp', description]
...
def main():
    ...
    for chapter_index in range(10):
        ...
        target = ROOT / 'data' / f'book-07-chapter-{chapter_index + 1:02d}.json'
